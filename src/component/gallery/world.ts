  ...
  constructor(base: Experience) {
    ...
    this.base.am.on("ready", async () => {
        ...
        if (maku.mesh instanceof THREE.Mesh) { 
          ...
          maku.mesh.addEventListener("click", () => {
            ...
            if (!this.currentActiveMesh) {
              ...
              const that = this;
              const materials = Array.isArray(maku.mesh.material) ? maku.mesh.material : [maku.mesh.material]
              materials.forEach((material) => {
      ...
      this.base.container.addEventListener("click", () => {
        if (this.currentActiveMesh) {
          const that = this;
          const materials = Array.isArray(this.currentActiveMesh.material) ? this.currentActiveMesh.material : [this.currentActiveMesh.material]
          materials.forEach((material) => {
            gsap.to((material as THREE.ShaderMaterial).uniforms.uProgress, {
              value: 0,
              duration: 1,
