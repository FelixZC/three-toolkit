...
async function addPhysicsTest(demo, world) {
    ...
    const planeModel = await loadGltfModelFunc('src/model/gltf/plane.gltf', [-10, 30, -10], [0.25, 0.25, 0.25], 1)
    const {
        gltfBody: planeGltfBody,
        gltfBodyMaterial: planeGltfBodyMaterial
    } = addPhysicsForModel(planeModel)
    demo.scene.add(planeModel);
    world.addBody(planeGltfBody);
    //添加房子
    const houseModel = await loadGltfModelFunc('src/model/gltf/littlest-tokyo.glb', [0, 2.2, 0], [0.01, 0.01, 0.01], 1)
    const {
