...
export default class ThreeDemo {
  ...
  setUpGUI() {
    ...
    fogFarController.onChange((value: number) => {
      (this.scene.fog as THREE.Fog).far = value;
    });
  }
   ...
   */
  addPlaneHelper(plane: THREE.Plane, size = 1, hexColor1 = 0xffff00) {
    const planeHelper = new THREE.PlaneHelper(plane, size, hexColor1);
   ...
   */
  addPointLightHelper(light: THREE.PointLight, sphereSize = 0.75) {
   ...
   */
  addSpotLightHelper(light: THREE.Light, sphereSize = 0.5) {
  ...
  setUpRenderer() {
    ...
    this.renderer.setClearColor((this.scene.fog as THREE.Fog).color);
  ...
  setUpScene() {
    ...
    this.scene.fog = new THREE.Fog(0xffffff, 0.2, 0);
    // // 设置场景的背景颜色
   ...
   */
  setUpSky() {
    const { scene, camera, renderer, gui } = this;
